 ...
 * 路由级代码分割。
 *
 * 原先是单 chunk 全量加载（约 685 KB），首屏必须解析完 team/codex2api 这些重页面才能渲染。
...
const Codex2ApiPage = lazy(() => import('@/pages/codex2api').then((m) => ({ default: m.Codex2ApiPage })));
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-sm text-muted-foreground">toCodex2 控制台加载中…</div>
        </div>
      </div>
    );
  }
...
const codex2apiRoute = createRoute({
  getParentRoute: () => authLayoutRoute,
  path: '/codex2api',
  component: withLayout('Codex2API 管理', Codex2ApiPage),
});
...
if (typeof window !== 'undefined') {
  window.addEventListener('tocodex2:unauthorized', () => {
