...
export async function api<T>(path: string, init?: RequestOptions): Promise<T> {
  ...
  if (res.status === 401 && !path.startsWith('/auth/')) {
    window.dispatchEvent(new CustomEvent('tocodex2:unauthorized'));
...
export const ERROR_LABELS: Record<string, string> = {
  ...
  CODEX2API_NOT_CONFIGURED: '请先配置 codex2api',
  CODEX2API_UNAVAILABLE: 'codex2api 连接失败',
  CODEX2API_PROXY_CREATE_FAILED: 'codex2api 代理创建失败',
