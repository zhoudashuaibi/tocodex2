...
import type {
  ...
  Codex2ApiConfigView,
  Codex2ApiMonitorLog,
  Codex2ApiMonitorView,
  Codex2ApiProxyReplaceResult,
  Codex2ApiProxyView,
  Codex2ApiSyncResult,
  TeamCard,
...
/** 账号导出格式 */
export type AccountExportFormat = 'tocodex2' | 'codex2api' | 'source';
...
export const accountsApi = {
  ...
  batchUpload: (ids: number[], options?: UploadOptions, order?: UploadOrder) =>
    api<{ created: number; updated: number; failed: { id: number; email: string | null; error: string }[]; updated_account_ids: number[] }>(
      '/accounts/batch-upload-codex2api',
...
// ---------- codex2api ----------
export const codex2apiApi = {
  config: () => api<Codex2ApiConfigView>('/codex2api/config'),
  updateConfig: (body: Record<string, unknown>) => api<{ config: Codex2ApiConfigView }>('/codex2api/config', {
    method: 'PUT',
    json: body,
  }),
  test: (body: { base_url?: string; admin_key?: string } = {}) =>
    api<{ ok: boolean; groups: number; latency_ms: number }>('/codex2api/test', { json: body }),
  groups: () => api<{ items: { id: number; name: string; status: string }[] }>('/codex2api/groups'),
  proxies: () => api<{ items: Codex2ApiProxyView[] }>('/codex2api/proxies'),
  replaceProxies: (body: { text: string; protocol: string; delete_old: boolean }) =>
    api<Codex2ApiProxyReplaceResult>('/codex2api/proxies/replace', { json: body }),
  remoteAccount: (email: string) =>
    api<{ found: boolean; account: { id: number; name: string; status: string; error_message: string | null } | null }>(
      `/codex2api/remote-accounts?email=${encodeURIComponent(email)}`,
    ),
  monitor: () => api<Codex2ApiMonitorView>('/codex2api/monitor'),
  monitorLogs: (limit = 20) => api<{ items: Codex2ApiMonitorLog[] }>(`/codex2api/monitor/logs?limit=${limit}`),
  updateMonitor: (body: Record<string, unknown>) => api<Codex2ApiMonitorView>('/codex2api/monitor', { json: body }),
  checkNow: () => api<{ ok: boolean; monitor: Codex2ApiMonitorView }>('/codex2api/monitor/check', { json: {} }),
  syncRemote: () => api<Codex2ApiSyncResult>('/codex2api/sync-remote', { method: 'POST', json: {} }),
};
