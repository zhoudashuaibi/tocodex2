...
/**
 * Codex 指纹收敛选项：取值与 codex2api 账号 extra.codex_fingerprint_mode 一一对应
 * （codex2api 账号编辑页同款四档）。off = 不写该 extra 键，原样透传客户端设备/会话标识。
 */
...
export const CODEX_FINGERPRINT_MODE_HINT =
  '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级恒定值，减少上游可见的设备数和会话数。默认关闭（原样透传客户端标识），需要时再显式开启；部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。新增账号时写入 codex2api extra，已有账号走「替换凭据」分支不改动该项（与禁用 5h/7d 暂停等同口径）。';
