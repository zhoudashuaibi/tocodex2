...
import { codex2apiApi } from '@/api';
import { errorMessage } from '@/api/client';
import type { CodexFingerprintMode, Codex2ApiMonitorLog, Codex2ApiMonitorLogItem, Codex2ApiProxyReplaceResult } from '@/api/types';
import {
...
export function Codex2ApiPage() {
  const queryClient = useQueryClient();
  const { data: config } = useQuery({
    queryKey: ['codex2api', 'config'],
    queryFn: () => codex2apiApi.config(),
  });
  const { data: monitor } = useQuery({
    queryKey: ['codex2api', 'monitor'],
    queryFn: () => codex2apiApi.monitor(),
    refetchInterval: 30_000,
  });
  const { data: monitorLogs } = useQuery({
    queryKey: ['codex2api', 'monitor', 'logs'],
    queryFn: () => codex2apiApi.monitorLogs(20),
  ...
  const [baseConcurrency, setBaseConcurrency] = useState('');
  ...
  const { data: groups } = useQuery({
    queryKey: ['codex2api', 'groups'],
    queryFn: () => codex2apiApi.groups(),
    enabled: Boolean(config?.base_url),
    retry: false,
  });
  const { data: remoteProxies } = useQuery({
    queryKey: ['codex2api', 'proxies'],
    queryFn: () => codex2apiApi.proxies(),
    enabled: Boolean(config?.base_url),
    retry: false,
  });
  ...
  useEffect(() => {
    if (config && !loaded) {
      ...
      setBaseConcurrency(ud.base_concurrency != null ? String(ud.base_concurrency) : '');
      setScoreBias(ud.score_bias != null ? String(ud.score_bias) : '');
      setSkipRefresh(ud.skip_refresh !== false);
      setModelWhitelist((ud.model_whitelist ?? []).join(', '));
      setAutoSelectProxy(ud.auto_select_proxy !== false);
      setProxyUrl(ud.proxy_url ?? '');
  ...
  const uploadDefaultsState = {
    base_concurrency: baseConcurrency === '' ? null : Number(baseConcurrency),
    score_bias: scoreBias === '' ? null : Number(scoreBias),
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('配置已保存');
      setAdminKey('');
      queryClient.invalidateQueries({ queryKey: ['codex2api'] });
    },
  ...
  const saveMonitorMutation = useMutation({
    mutationFn: () => codex2apiApi.updateMonitor(monitorConfigState),
    onSuccess: () => {
      toast.success(monitorEnabled ? '监控已启用' : '监控已停用');
      queryClient.invalidateQueries({ queryKey: ['codex2api', 'monitor'] });
  ...
  const checkMutation = useMutation({
    mutationFn: () => codex2apiApi.checkNow(),
    onSuccess: ({ monitor: view }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['codex2api', 'monitor'] });
      queryClient.invalidateQueries({ queryKey: ['codex2api', 'monitor', 'logs'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard'] });
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  const testMutation = useMutation({
    mutationFn: () => codex2apiApi.test(baseUrl ? { base_url: baseUrl, admin_key: adminKey || undefined } : {}),
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="text-sm text-muted-foreground">
            当前 codex2api 内 {(remoteProxies?.items ?? []).length} 个代理 · 绑定账号合计{' '}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              <Label>每号并发（留空不覆盖）</Label>
              <Input value={baseConcurrency} onChange={(e) => setBaseConcurrency(e.target.value)} placeholder="10" />
            </div>
            <div className="space-y-1.5">
              <Label>调度偏置（-200~200）</Label>
              <Input value={scoreBias} onChange={(e) => setScoreBias(e.target.value)} placeholder="余额分档" />
              <p className="text-xs text-muted-foreground">正向=优先调度；留空按余额分档：≤10 刀 → +40，11-19 刀 → +20，20-39 刀 → +30，≥40 刀 → +10</p>
            </div>
            <div className="space-y-1.5">
              <Label>创建后跳过上游刷新</Label>
              <label className="flex h-9 items-center gap-2 text-sm">
                <Switch checked={skipRefresh} onCheckedChange={setSkipRefresh} />
                {skipRefresh ? '跳过（推荐）' : '立即刷新'}
              </label>
              <p className="text-xs text-muted-foreground">批量上传时立即刷新会把 codex2api 网关打卡，保持开启由其后台调度器慢慢刷</p>
            </div>
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2 text-sm">
              ...
              自动绑定 codex2api 内绑定数最少的代理
            </label>
            {!autoSelectProxy && (
              <Select value={proxyUrl || '__none__'} onValueChange={(value) => setProxyUrl(value === '__none__' ? '' : value)}>
                  ...
                  <SelectGroup>
                    ...
                    {(remoteProxies?.items ?? []).map((proxy) => (
                      <SelectItem key={proxy.id} value={proxy.url}>
                        {proxy.name || proxy.url}（绑定 {proxy.account_count}）
                      </SelectItem>
                    ))}
                  </SelectGroup>
                </SelectContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <label className="flex items-center gap-2 text-sm">
            ...
            巡检时刷新已上传号的余额（优先经 codex2api 绑定代理查询）
          </label>
          {refreshBalance && (
          ...
          <label className="flex items-center gap-2 text-sm">
            ...
            {replenishMode === 'resource'
              ...
              : '低于保底自动补号：codex2api 缺号优先上传主池库存，主池库存低于「主池库存保底」再从备用池登录补入'}
          </label>
            ...
            {replenishMode === 'resource' && (
              <>
                ...
                <p className="text-xs text-muted-foreground md:col-span-2">
                  统计范围为 codex2api 在架的本系统主池号（远端非 error，限流/暂停调度/待重授均计入），两项任一不达标即触发补号；
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label>上传顺序（主池库存 → codex2api）</Label>
...
function ReplaceProxyDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const [result, setResult] = useState<Codex2ApiProxyReplaceResult | null>(null);

  const replaceMutation = useMutation({
    mutationFn: () => codex2apiApi.replaceProxies({ text, protocol, delete_old: deleteOld }),
    onSuccess: (data) => {
      setResult(data);
      setConfirming(false);
      ...
      queryClient.invalidateQueries({ queryKey: ['codex2api', 'proxies'] });
    },
    onError: (error) => {
      setConfirming(false);
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title="确认一键更换代理 IP？"
        description="将创建新代理（label 自动续接编号），并把绑定在现有代理上的全部账号随机改绑到新代理；改绑全部成功后删除旧代理（存在改绑失败时保留旧代理，防止账号裸奔）。此操作不可撤销。"
        confirmText="确认替换"
...
function ReplaceResultView({ result }: { result: Codex2ApiProxyReplaceResult }) {
  const shortUrl = (url: string) => url.replace(/^[a-z0-9]+:\/\//i, '').replace(/^[^@]*@/, '');
  const problems = [
    ...result.create_failed.map((item) => `创建失败 ${item.proxy}：${item.reason}`),
    ...result.rebound.failed_groups.map((group) => `改绑失败 ${group.name || shortUrl(group.proxy_url)}（${group.count} 个账号）：${group.reason}`),
    ...result.old_proxies.skipped.map((item) => `旧代理 ${item.name} 未删除：${item.reason}`),
      ...
      {result.rebound.groups.length > 0 && (
        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs">
          {result.rebound.groups.map((group) => (
            <span key={group.proxy_url}>
              {group.name || shortUrl(group.proxy_url)} × {group.count}
            </span>
          ))}
        </div>
      )}
...
function MonitorSummaryChips({ summary }: { summary: Codex2ApiMonitorLog['summary'] }) {
...
function MonitorLogItemRow({ item }: { item: Codex2ApiMonitorLogItem }) {
...
function durationSeconds(log: Codex2ApiMonitorLog): number | null {
