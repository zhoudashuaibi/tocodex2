  ...
  return (
    <div className="space-y-4">
      ...
      <ListShell
        ...
        emptyIcon={Inbox}
        emptyTitle="备用号池为空"
        emptyDescription="导入 codex2api 账号导出 JSON（notes 含邮箱四段信息、ChatGPT 密码、两步验证），系统将自动补全凭据并初始化余额与封禁状态"
      ...
      <ImportDialog
        ...
        title="导入账号"
        description="导入 codex2api 账号导出 JSON：邮箱四段信息、ChatGPT 密码、两步验证密钥一次性补全，全部进入备用号池"
        placeholder={[
          'codex2api 账号导出 JSON（accounts[].notes 携带全部凭据）：',
        ...
        <div className="flex items-center gap-3 rounded-md border border-[var(--warning)]/40 bg-[var(--warning)]/10 p-3 text-sm">
          <span className="flex-1">
            {importResult.duplicates_remote.length > 0 &&
              `${importResult.duplicates_remote.length} 个账号已在远端 codex2api：可在导入详情中「收编进主号池」（直接关联远端，不重新登录）。`}
