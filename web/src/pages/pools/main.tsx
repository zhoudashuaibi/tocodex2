...
import { accountsApi, codex2apiApi } from '@/api';
...
export function MainPoolPage() {
  ...
  const syncRemoteMutation = useMutation({
    mutationFn: () => codex2apiApi.syncRemote(),
    onSuccess: (result) => {
    ...
    <div className="space-y-4">
          ...
          onClick={() =>
            download(
              selection.count > 0 && selection.count <= 500
                ? accountsApi.exportUrl({ ids: selection.selectedIds, format: 'tocodex2' })
                : accountsApi.exportByFilterUrl({ ...filterForIds, format: 'tocodex2' }),
              'tocodex2-accounts.json',
      ...
      >
        {items.map((account) => (
          <TableRow key={account.id} data-state={selection.isSelected(account.id) ? 'selected' : undefined}>
            ...
            <TableCell className="text-xs text-muted-foreground">{formatRelativeTime(account.codex2api_uploaded_at)}</TableCell>
      ...
      >
        ...
        <Button
          size="sm"
          variant="outline"
          onClick={() => download(accountsApi.exportUrl({ ids: selection.selectedIds, format: 'codex2api' }), 'codex2api-import.json')}
        >
          导出(codex2api)
        </Button>
        <Button
...
function BalanceEstimateDialog({
  ...
  const reasonLabel: Record<string, string> = {
    not_uploaded: '未上传到 Codex2API',
    remote_account_not_found: 'Codex2API 中未找到账号',
    initial_balance_unknown: '初始化余额未知',
    remote_used_amount_unknown: 'Codex2API 未提供明确已用金额',
  };
    ...
    <Dialog open={Boolean(estimate)} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            使用 Codex2API 管理端账号统计中的累计费用减去本地初始化余额（统计最近 90 天），不会调用 OpenAI 余额接口或刷新账号授权。
          </DialogDescription>
        </DialogHeader>
        {estimate && (
          <div className="space-y-4">
            ...
            <div className="text-xs text-muted-foreground">本地初始化余额缺失时，使用 Codex2API 账号名末尾的 `---N` 整数美元后缀；该后缀来自上传时的邮件余额。</div>
...
function UploadConfigDialog({
  ...
  useEffect(() => {
    if (open && config && !loaded) {
      ...
      setBaseConcurrency(defaults.base_concurrency != null ? String(defaults.base_concurrency) : '');
      setScoreBias(defaults.score_bias != null ? String(defaults.score_bias) : '');
      setSkipRefresh(defaults.skip_refresh !== false);
      setModelWhitelist((defaults.model_whitelist ?? []).join(', '));
      setProxyUrl(defaults.proxy_url ?? '');
      setLoaded(true);
    }
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>尚未配置 codex2api</DialogTitle>
            <DialogDescription>请先完成 codex2api 连接配置后再上传账号。</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button asChild>
              <Link to="/codex2api">前去配置</Link>
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>上传 {count} 个账号到 codex2api</DialogTitle>
