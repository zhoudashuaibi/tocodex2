...
import { codex2apiApi, teamApi } from '@/api';
  ...
  return (
    <div className="space-y-4">
                    ...
                    <TableRow key={account.id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {account.codex2api_account_id ? (
                          <>
                            <Badge variant="secondary" className="mr-1.5 py-0">#{account.codex2api_account_id}</Badge>
                            {formatRelativeTime(account.codex2api_uploaded_at)}
                          </>
                        ) : (
                          '—'
                        )}
      ...
      <ConfirmDialog
        ...
        title={`上传 ${selectedAccounts.size} 个账号到 codex2api？`}
...
function TeamConfigSection() {
  ...
  const { data: codex2apiConfig } = useQuery({ queryKey: ['codex2api', 'config'], queryFn: () => codex2apiApi.config() });
  const { data: groups } = useQuery({
    queryKey: ['codex2api', 'groups'],
    queryFn: () => codex2apiApi.groups(),
    enabled: Boolean(codex2apiConfig?.base_url),
    retry: false,
  });
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>30d.team 兑换 / 401 找回服务地址；codex2api 连接与代理 IP 一键更换复用「Codex2API」页配置</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <label className="flex items-center gap-2 text-sm">
            ...
            凭据更新后自动上传 codex2api（含首次提取与 401 找回）
          </label>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex max-h-32 flex-wrap gap-2 overflow-y-auto rounded-md border p-2">
              {!codex2apiConfig?.base_url && (
                <span className="text-xs text-muted-foreground">请先在「Codex2API」页配置连接后再选择分组</span>
              )}
              {codex2apiConfig?.base_url && (groups?.items ?? []).length === 0 && (
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={scoreBias} onChange={(e) => setScoreBias(e.target.value)} placeholder="留空不偏置" />
            </div>
            <div className="space-y-1.5">
