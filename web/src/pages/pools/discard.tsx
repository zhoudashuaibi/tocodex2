...
export function DiscardPoolPage() {
  ...
  /** 同步 codex2api 用量：有选中只同步选中，否则同步当前筛选下待同步的账号 */
  ...
  const handleExport = () => {
    download(exportUrl, 'tocodex2-discard.json').catch((error) => toast.error(errorMessage(error)));
  };
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-md bg-muted/60 px-4 py-2.5 text-sm text-muted-foreground">
        ...
        初始化的值（与备用池同源，未拿到时显示「未查询」）；「已用额度」取自 codex2api 账号用量统计，
      ...
      <ListToolbar>
        ...
        <Tooltip>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const ok = window.confirm(
                  `将对${scope}账号逐个查询 codex2api 用量统计（90 天），可能需要数秒${
          ...
          <TooltipContent>
            读取 codex2api 管理端账号的累计已用额度并保存快照；账号被废弃时已自动抓取过一次。
      ...
      <ListShell
        ...
        emptyDescription={
          reason
            ? '换个原因试试，或点击当前徽章取消筛选'
            : '被 codex2api 监控判定 401/429 或手动废弃的账号会出现在这里'
        }
 ...
 * 废弃时的出口代理：名字 + 认证账号。
 *
 * 两截都有意义：代理名（codex2api 里的编号）说明是哪台机器，
...
function DiscardProxyCell({ account }: { account: DiscardAccount }) {
  ...
  const user = account.proxy_user;  if (!name && !user) {
    return (
      <Tooltip>
        <TooltipTrigger asChild>
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <div className="max-w-[150px] cursor-help font-mono text-xs leading-tight">
          {/* codex2api 代理是 URL 实体（快照 id 恒空），label 缺失时用认证账号当标题 */}
          <div className="truncate">{name ?? `@${user ?? '?'}`}</div>
          {user && name && <div className="truncate text-muted-foreground">认证账号 {user}</div>}
        </div>
      </TooltipTrigger>
      <TooltipContent className="max-w-md">
        <div>代理：{name ?? `@${user ?? '?'}`}</div>
 ...
 * （codex2api 契约里 off 就是不写这个 extra 键），跟「没抓到」是两回事，
  ...
  if (!mode) {
    return (
      <Tooltip>
        ...
        <TooltipContent className="max-w-md">
          没有这条号的收敛档位记录：本列是废弃那一刻从 codex2api 远端账号 extra 抓的快照。
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent className="max-w-md">
        ...
        <div className="text-muted-foreground">
          废弃当时抓取（{formatDateTime(account.codex_fingerprint_at)}），取自 codex2api 账号
  ...
  if (account.used_amount === null) {
    return (
      <Tooltip>
        ...
        <TooltipContent>点右上角「同步远端用量」从 codex2api 读取累计已用额度</TooltipContent>
      </Tooltip>
    );
  }
  return (
    <Tooltip>
      ...
      <TooltipContent>
        ...
        <div className="text-muted-foreground">
          取自 codex2api 账号累计用量。codex2api 不提供历史时点查询，账号被废弃时已自动抓取一次
        </div>
      </TooltipContent>
    </Tooltip>
  );
  ...
  const failed = result.items
    ...
    .map((item) => {
      ...
      return {
        ...
        label: item.email,
        // 查询失败时带上服务端返回的具体原因，便于区分「远端没有」与「codex2api 连不上」
        reason: item.detail ? `${label}：${item.detail}` : label,
      };
    });

  if (summary.not_linked) notes.push(`${summary.not_linked} 个账号从未上传 codex2api，无远端用量可查`);
  if (summary.remote_account_not_found) notes.push(`${summary.remote_account_not_found} 个账号在 codex2api 中已不存在`);
  if (summary.remote_used_amount_unknown) notes.push(`${summary.remote_used_amount_unknown} 个账号远端未提供用量字段`);
  if (summary.fetch_failed) notes.push(`${summary.fetch_failed} 个账号查询 codex2api 失败（详情见下方失败列表）`);
