 ...
 *
 * 保留规则：本地 codex2api_account_id 指向的那份优先保留，其次是非 error 状态，最后取最小 id。
 */
...
const { createCodex2apiClient } = await import(
  pathToFileURL(path.join(__dirname, '..', 'server', 'modules', 'codex2api', 'client.js')).href
);

// 只读打开：脚本不写库、不跑迁移，可以对正在运行的实例直接执行
const dbPath = path.join(dataDir, 'tocodex2.db');
...
const crypto = createCrypto({ dataDir, secretKeyEnv: process.env.TOCODEX2_SECRET_KEY || '' });

function getConfig() {
  const row = db.prepare('SELECT value, encrypted FROM settings WHERE key = ?').get('codex2api.config');
  if (!row) return null;
  const text = row.encrypted ? crypto.decrypt(row.value, 'settings.codex2api.config') : row.value;
  try {
...
function pickKeeper(copies, local) {
  const linked = Number(local?.codex2api_account_id);
...
if (!config?.base_url || !config?.admin_key) {
  console.error('codex2api 未配置（settings.codex2api.config 缺少 base_url / admin_key），无法扫描。');
  process.exit(1);
}
console.log(`  codex2api: ${config.base_url}`);

const client = createCodex2apiClient(() => config);
...
for (const row of db.prepare(`SELECT id, email, pool, status, codex2api_account_id FROM accounts`).all()) {
...
for (const target of targets) {
  const localText = target.local
    ? `本地 #${target.local.id} [${target.local.pool}/${target.local.status}] linked=${target.local.codex2api_account_id ?? '-'}`
    : '本地无记录';
...
if (!apply) {
  ...
  console.log('若接口不支持 DELETE，请到 codex2api 后台按上面的 id 手工删除。');
  db.close();
  process.exit(0);
}
...
if (failed.length) {
  console.log('失败列表（可在 codex2api 后台手工删除）：');
