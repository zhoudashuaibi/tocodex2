...
const DEFAULT_CODEX2API_OUT = "tmp/codex2api-import-oauth.json";
...
function enableJsonEvents() {
  ...
  JSON_EVENTS.attempt = Math.max(1, Number.parseInt(process.env.TOCODEX2_JOB_ATTEMPT || "1", 10) || 1);
...
async function run() {
  ...
  if (!["both", "session", "codex2api"].includes(outputMode)) {
    throw new Error("--output-mode must be one of: both, session, codex2api");
  }
  const outPath = path.resolve(args.out || DEFAULT_OUT);
  const codex2apiOutPath = path.resolve(args.codex2apiOut || DEFAULT_CODEX2API_OUT);
  ...
  const transport = new TlsFingerprintTransport({
    ...
    profile:
      args.tlsProfile
      || process.env.TOCODEX2_TLS_PROFILE
      || (proxyTemplate || args.refreshCodex2api ? "chrome146" : "auto"),
  ...
  try {
    if (args.refreshCodex2api) {
      emitEvent("starting", {
        mode: "refresh",
        email: await guessRefreshEmail(args.refreshCodex2api),
      });
      ...
      const sourcePath = path.resolve(args.refreshCodex2api);
      const targetPath = path.resolve(args.codex2apiOut || sourcePath);
      const refreshed = await refreshCodex2apiOauthExport({
        authBase,
        sourcePath,
        targetPath,
      ...
      console.log(`[ok] Saved codex2api import: ${targetPath}`);
    ...
    const rl = args.jsonEvents ? null : readline.createInterface({ input, output });
    const checkpointPath = path.resolve(args.checkpoint || args.resumeCheckpoint || `${codex2apiOutPath}.checkpoint.json`);
    try {
    let client = null;
    ...
    flowLoop: while (true) {
      ...
      if (codex?.callbackUrl && codex?.codeVerifier && !args.noCodex2apiExport && outputMode !== "session") {
        console.log("[6/6] Convert OAuth callback to codex2api import");
        emitStageEvent("finalizing");
        let codex2apiExport;
        try {
          codex2apiExport = await buildCodex2apiOauthExport({
            authBase,
            ...
            accountName: args.codex2apiName,
        ...
        await writeJsonAtomic(codex2apiOutPath, codex2apiExport.data);
        await removeProtocolCheckpoint(checkpointPath);
        console.log(`[ok] Saved codex2api import: ${codex2apiOutPath}`);
        console.log(`[ok] Account: ${codex2apiExport.account.name}`);
        console.log(`[ok] Email: ${codex2apiExport.account.credentials.email || "<unknown>"}`);
        console.log(`[ok] chatgpt_account_id: ${mask(codex2apiExport.account.credentials.chatgpt_account_id)}`);
        console.log(`[ok] chatgpt_user_id: ${mask(codex2apiExport.account.extra.chatgpt_user_id)}`);
        console.log("[note] codex2api import JSON contains access_token, refresh_token and id_token. Do not share it.");
        emitEvent("result_saved", {
          path: codex2apiOutPath,
          account: {
            email: codex2apiExport.account.credentials.email || "",
            name: codex2apiExport.account.name,
            chatgpt_account_id: codex2apiExport.account.credentials.chatgpt_account_id || "",
          },
        });
      } else if (outputMode === "codex2api") {
        throw new Error("Cannot output only codex2api import without Codex OAuth. Remove --web-only and --no-codex2api-export.");
...
async function loginChatgptWeb(client, { chatgptBase, authBase, email, rl, password, totpSecret, totpPickupUrl }) {
  ...
  if (authPage.last?.text) {
    ...
    // 诊断钩子：TOCODEX2_DEBUG_AUTH_HTML=<path> 时把认证页原始 HTML 落盘
    const htmlDumpPath = process.env.TOCODEX2_DEBUG_AUTH_HTML;
    if (htmlDumpPath) {
...
async function dynamicSentinelTokens(client, { authBase, deviceId, flow }) {
  if (process.env.NODE_ENV === "test" && process.env.TOCODEX2_TEST_SENTINEL_TOKEN) {
    return {
      token: JSON.stringify({
        p: "test-proof",
        t: null,
        c: process.env.TOCODEX2_TEST_SENTINEL_TOKEN,
        id: deviceId,
        flow,
      }),
      soToken: process.env.TOCODEX2_TEST_SENTINEL_SO_TOKEN || null,
    };
  }
...
async function buildCodex2apiOauthExport({
  authBase,
  callbackUrl,
  codeVerifier,
  clientId,
...
async function refreshCodex2apiOauthExport({ authBase, sourcePath, targetPath, fallbackClientId, transport }) {
  let data;
  try {
    data = JSON.parse(await fs.readFile(sourcePath, "utf8"));
  } catch (error) {
    throw new Error(`REFRESH_TOKEN_INVALID: 无法读取原 codex2api 文件：${error.message}`);
  }
  const account = data?.accounts?.[0];
  const credentials = account?.credentials;
  const refreshToken = credentials?.refresh_token;
  const clientId = account?.extra?.client_id || fallbackClientId;
  if (data?.type !== "codex2api-data" || !account || !refreshToken) {
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (item.startsWith("--codex2api-out=")) args.codex2apiOut = item.slice("--codex2api-out=".length);
    else if (item === "--codex2api-out") args.codex2apiOut = argv[++i];
    ...
    else if (item === "--no-codex2api-export") args.noCodex2apiExport = true;
    else if (item.startsWith("--refresh-codex2api=")) args.refreshCodex2api = item.slice("--refresh-codex2api=".length);
    else if (item === "--refresh-codex2api") args.refreshCodex2api = argv[++i];
    ...
    else if (item.startsWith("--codex2api-name=")) args.codex2apiName = item.slice("--codex2api-name=".length);
    else if (item === "--codex2api-name") args.codex2apiName = argv[++i];
...
Options:
  ...
  --codex2api-out <file>            codex2api import JSON path. Default: ${DEFAULT_CODEX2API_OUT}
  --output-mode <mode>            both, session, or codex2api. Default: both
  --no-codex2api-export             Do not exchange OAuth code or write codex2api import JSON.
...
Notes:
  ...
  TOCODEX2_JOB_ATTEMPT sets the attempt number embedded in every event.
