...
export class TlsFingerprintTransport {
  ...
  async ensureChild() {
    ...
    if (!python) {
      throw new Error("未找到可用的 Python curl_cffi 环境，请先运行 python -m pip install -r requirements.txt；也可以设置 TOCODEX2_PYTHON 指定 Python 路径");
    }
...
function findPythonCommand() {
  const configured = String(process.env.TOCODEX2_PYTHON || "").trim();
