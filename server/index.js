...
import { createCodex2apiModule } from './modules/codex2api/index.js';
...
const logger = createLogger(process.env.TOCODEX2_LOG_LEVEL || 'info');
...
const codex2apiRegister = fp(async (f) => {
  await createCodex2apiModule({ engine: jobsEngine, logger })(f);
}, { name: 'codex2api' });
...
await app.register(codex2apiRegister);
...
// 启动任务引擎调度循环 + codex2api 监控（若启用）
jobsEngine.start();
app.codex2apiMonitor?.startIfEnabled?.();
...
try {
  ...
  logger.info(`tocodex2 v${config.version} listening on http://${config.host}:${config.port} (data: ${config.dataDir})`);
} catch (error) {
