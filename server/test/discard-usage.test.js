...
/** 与 modules/codex2api/client.js accountUsedAmount 同语义的最小实现（测试替身）。 */
...
test('双路回退：codex2api_account_id 命中优先于 email', () => {
  const index = indexOf(
    { id: 7, credentials: { email: 'a@test.local' }, used_amount: 1 },
    { id: 42, credentials: { email: 'a@test.local' }, used_amount: 9 },
  );
  const remote = resolveRemoteAccount({ codex2api_account_id: 42, email: 'a@test.local' }, index);
  assert.equal(remote.id, 42);
});

test('codex2api_account_id 陈旧时回退 email 命中（废弃后不再参与远端同步）', () => {
  // 远端账号被删除重建过：本地记录的 id 已不存在，但邮箱仍能命中
  const index = indexOf({ id: 99, credentials: { email: 'b@test.local' }, used_amount: 3.5 });
  const remote = resolveRemoteAccount({ codex2api_account_id: 12, email: 'b@test.local' }, index);
  assert.equal(remote.id, 99);

  const resolved = classifyDiscardUsage({
    row: { codex2api_account_id: 12, email: 'b@test.local' },
    remote,
...
test('email 匹配大小写不敏感', () => {
  ...
  const remote = resolveRemoteAccount({ codex2api_account_id: null, email: 'mixed@test.local' }, index);
  assert.equal(remote?.id, 1);
});

test('从未上传：reason=not_linked，不写数值', () => {
  const resolved = classifyDiscardUsage({
    row: { codex2api_account_id: null, email: 'c@test.local' },
    remote: null,
    used: null,
  });
...
test('曾关联但远端已无此号：reason=remote_account_not_found', () => {
  const resolved = classifyDiscardUsage({
    row: { codex2api_account_id: 5, email: 'd@test.local' },
    remote: null,
    used: null,
  });
...
test('查询过程报错且查不到 → fetch_failed，并带上原始原因', () => {
  const resolved = classifyDiscardUsage({
    ...
    remote: null,
    used: null,
    lookupError: 'codex2api 请求超时（120s）',
  });
...
test('远端存在但账号对象与统计都没有用量字段：reason=remote_used_amount_unknown', () => {
  ...
  const resolved = classifyDiscardUsage({
    row: { codex2api_account_id: 8, email: 'e@test.local' },
    remote,
...
test('用量为 0 视为有效值（不是未知）', () => {
  ...
  const resolved = classifyDiscardUsage({
    row: { codex2api_account_id: 9, email: 'f@test.local' },
    remote,
    used: usedFrom(remote),
  });
  assert.equal(resolved.reason, null);
...
test('用量来自统计接口的 summary.total_cost（回归：账号对象本身没有费用字段）', () => {
  // 真实 codex2api 的账号对象不含累计消费，只有 /stats 的 summary.total_cost 才有；
  ...
  const resolved = classifyDiscardUsage({
    row: { codex2api_account_id: 10, email: 'g@test.local' },
    remote,
    used,
  });
