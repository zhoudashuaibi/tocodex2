...
/** 远端单账号接口 + 代理列表：真实形状（账号只带 proxy_url，代理 label 在代理表） */
function fakeRemoteSync({ remoteById = {}, proxies = [], failGetAccount = false } = {}) {
  const byUrl = new Map(proxies.map((item) => [String(item.url), item]));
  return {
    // 与 remote-sync 的真实接口一致：一次解析同时给出代理与 Codex 指纹收敛档位。
    // fake 只负责代理那半边（档位由平铺字段决定，这里不伪造），保持既有断言语义不变。
    resolveDiscardRemote: async ({ accountId, email }) => {
      ...
      if (!proxyUrl) return { proxy: null, codex_fingerprint_mode: null };
      if (failGetAccount) throw new Error('codex2api 连不上');
      const proxy = byUrl.get(proxyUrl) || {};
      let username = null;
      try {
        username = new URL(proxyUrl).username ? decodeURIComponent(new URL(proxyUrl).username) : null;
      } catch {
        username = null;
      }
      return {
        proxy: { id: proxy.id ?? null, name: proxy.label ?? null, username },
        codex_fingerprint_mode: null,
      };
    },
    resolveProxyLabel: async (proxyUrl) => byUrl.get(String(proxyUrl || '').trim())?.label ?? null,
  };
}

test('巡检废弃：用废弃当下同步过来的远端绑定代理落库（代理 label + 认证账号）', async (t) => {
  const ctx = setup();
  t.after(() => ctx.db.close());
  const { pools } = buildHarness({
    ...ctx,
    remoteSync: fakeRemoteSync({
      proxies: [{ id: 3, url: 'http://u123:p@1.2.3.4:8080', label: '23' }],
    }),
  });

  // 巡检手里本来就有远端账号对象（带 proxy_url），原样传给 pools —— 不查远端单账号接口
  ...
  assert.equal(row.discard_proxy_name, '23', 'label 经代理索引补齐');
  assert.equal(row.discard_proxy_user, 'u123', '认证账号从 proxy_url 解析');
...
test('手动废弃：调用方没给代理 → 按 codex2api_account_id 单查远端补齐', async (t) => {
  const ctx = setup();
  t.after(() => ctx.db.close());
  ctx.db.prepare('UPDATE accounts SET codex2api_account_id=701 WHERE id=1').run();
  const remoteSync = fakeRemoteSync({
    remoteById: { 701: { id: 701, proxy_url: 'http://acct-9:p@5.6.7.8:8080' } },
    proxies: [{ id: 9, url: 'http://acct-9:p@5.6.7.8:8080', label: '12' }],
  });
...
test('远端查询失败：不阻断废弃，代理列留空而不是写错值', async (t) => {
  ...
  t.after(() => ctx.db.close());
  ctx.db.prepare('UPDATE accounts SET codex2api_account_id=701 WHERE id=1').run();
  const remoteSync = fakeRemoteSync({ remoteById: { 701: { id: 701, proxy_url: 'http://1.2.3.4:8080' } }, failGetAccount: true });
  const { pools } = buildHarness({ ...ctx, remoteSync });
...
test('废弃后又被移回主池：快照不覆盖，也不会写进主池的号', async (t) => {
  ...
  const { pools } = buildHarness({
    ...ctx,
    remoteSync: fakeRemoteSync({
      remoteById: { 701: { id: 701, proxy_url: 'http://1.2.3.4:8080' } },
      proxies: [{ id: 9, url: 'http://1.2.3.4:8080', label: '12' }],
    }),
  });
  ctx.db.prepare('UPDATE accounts SET codex2api_account_id=701 WHERE id=1').run();
