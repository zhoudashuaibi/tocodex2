...
function setup({ remoteAccounts = [] } = {}) {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tocodex2-adopt-'));
  ...
  app.decorate('settings', {
    get: (key) => (key === 'codex2api.config' ? { base_url: 'https://codex2api.test', admin_key: 'key' } : {}),
  });
  app.decorate('codex2apiClient', {
...
test('adopt_remote：本地无记录的远端账号收编进主号池，不登录、关联远端、禁自动修复', async () => {
  ...
  assert.equal(adopted.codex2api_account_id, 101);
  assert.equal(adopted.codex2api_status, 'active');
...
test('adopt_remote：备用池已有且远端也存在 → 升级进主号池，保留初始余额', async () => {
  ...
  assert.equal(upgraded.codex2api_account_id, 101);
...
test('adopt_remote：远端状态镜像（error 一并写入 codex2api_status）', async () => {
  await start([remoteAccount(202, 'a@test.com', 'error')]);
  await importCall({ text: LINE_A, adopt_remote: true });
  const adopted = accountRow('a@test.com');
  assert.equal(adopted.codex2api_status, 'error');
  assert.equal(adopted.codex2api_account_id, 202);
});
