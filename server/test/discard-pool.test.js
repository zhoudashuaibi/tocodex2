...
function seed(db) {
  const insert = db.prepare(
    `INSERT INTO accounts(id, email, pool, status, imported_at, discarded_at, discard_reason,
                          codex2api_account_id, discard_used_amount, discard_used_amount_at,
...
const remoteAccounts = [
  // 真实 codex2api 的账号对象不含累计消费，费用只在 /usage 接口的 total_account_billed 里，
  // 因此这里刻意不带费用字段 —— 任何忘记查 /usage 的实现都会拿不到值
  { id: 101, email: 'joined@test.local' },
  // 103 故意不存在：模拟远端账号已被删除
  // 2 号从未上传，也没有远端账号
];

/** 远端账号 id → /usage 返回的累计费用（缺 key 表示统计接口也查不到） */
const remoteStats = { 101: { total_account_billed: 12.5 } };

/**
 * 假客户端：与真实 createCodex2apiClient 保持同样的能力与响应形状。
 *
 * **刻意让 listAllAccounts 抛错** —— 早期实现依赖它建全量 email 索引，
 * 在账号量大的实例上会翻几十页后撞上 120s 超时，表现为「所有账号都失败」。
 * 任何回退到全量列表的代码都会在这里立刻暴露。
 */
function fakeCodex2apiClient(overrides = {}) {
  const byId = new Map(remoteAccounts.map((account) => [String(account.id), account]));
  const byEmail = new Map(remoteAccounts.map((account) => [account.email.toLowerCase(), account]));
  return {
    ...
    getAccount: async (id) => {
      ...
      // 远端已删除 → codex2api 返回 404，客户端据此抛错
      if (!hit) throw new Error('codex2api 返回 HTTP 404：账号不存在');
      return hit;
    },
    getAccountStats: async (id) => {
      const stats = remoteStats[String(id)];
      if (!stats) throw new Error('codex2api 返回 HTTP 404：统计数据不存在');
      return stats;
    },
    ...
    accountUsedAmount: (account) => {
      // 与真实 client.accountUsedAmount 同一张候选表：/usage 的 total_account_billed 优先，
      // 账号对象自带的 5h/7d 窗口 account_billed 次选
      const candidates = [
...
async function setup(t) {
  ...
  app.decorate('settings', { get: () => ({ base_url: 'http://codex2api.test', admin_key: 'k' }) });
  app.decorate('config', { dataDir: process.cwd() });
  app.decorate('codex2apiClient', fakeCodex2apiClient());
...
test('同步端点：分类统计（更新 / 远端无此号 / 未关联）并落库', async (t) => {
  ...
  assert.equal(updated.discard_used_amount_source, 'total_account_billed');
...
test('同步端点：不调用全量账号列表，只按目标账号解析（回归：大号池超时）', async (t) => {
  ...
  const { app } = await setup(t);
  // 换成一个会记账的客户端：fakeCodex2apiClient 的 listAllOpenAiAccounts 本来就会抛错
  const counting = fakeCodex2apiClient({
    getAccount: async (id) => {
      calls.getAccount += 1;
  ...
  // 1 号有 codex2api_account_id → 走单账号接口，不需要邮箱查找
...
test('同步端点：查询异常归为查询失败并带上具体原因', async (t) => {
  const { app } = await setup(t);
  app.codex2apiClient = fakeCodex2apiClient({
    getAccount: async () => {
      throw new Error('codex2api 请求超时（120s）');
    },
...
test('同步端点：远端存在但没用用量字段 → remote_used_amount_unknown', async (t) => {
  ...
  app.codex2apiClient = fakeCodex2apiClient({
    ...
    getAccount: async (id) => ({ id: Number(id), email: 'joined@test.local' }),
...
test('同步端点：统计接口故障时退回账号对象自带的用量字段', async (t) => {
  ...
  const account = {
    ...
    usage_5h_detail: { account_billed: 7 },
  };
  app.codex2apiClient = fakeCodex2apiClient({
  ...
  assert.equal(body.items[0].used_amount_source, 'usage_5h_detail.account_billed');
});

test('同步端点：未配置 codex2api 返回 422 VALIDATION 而不是 500', async (t) => {
  const db = new Database(':memory:');
  for (const migration of listMigrations()) db.exec(migration.sql);
  ...
  bare.decorate('codex2apiClient', fakeCodex2apiClient());
  ...
  assert.match(response.json().error.message, /codex2api/);
});
...
test('同步端点：搜索词与原因同样收敛范围，且与列表口径一致', async (t) => {
  ...
  assert.equal(searched.json().items[0].used_amount_source, 'total_account_billed');
...
test('按筛选导出：与列表同口径，不依赖 ids 列表', async (t) => {
  const { app } = await setup(t);
  const response = await app.inject('/api/v1/accounts/export-by-filter?pool=discard&reason=banned_401&format=tocodex2');
  assert.equal(response.statusCode, 200);
  assert.equal(response.headers['content-disposition']?.includes('tocodex2'), true);
