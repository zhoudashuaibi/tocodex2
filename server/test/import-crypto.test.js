...
test('crypto 信封加解密往返 + AAD 绑定', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tocodex2-crypto-'));
  try {
    ...
    // AAD 不匹配 → 解密失败
    assert.throws(() => crypto.decrypt(envelope, 'settings.codex2api.config'), /DECRYPT_FAILED/);
    // JSON 封面
    const json = { a: 1, b: 'x' };
...
test('密钥轮换后旧密文解密失败（tryDecrypt 返回 null）', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'tocodex2-crypto2-'));
  try {
