 ...
 *
 * 要守住的三件事：
 *  1. 值来自**废弃那一刻**的远端 codex_fingerprint_mode 平铺字段，而不是事后现查 ——
 *     远端档位随时能在 codex2api 账号编辑页改，号废弃后还可能被清理，错过那一刻就再也拿不到；
 *  2. codex2api 对 codex 渠道账号恒下发已归一的档位，off 是**有效值**；
 *     只有字段压根缺失（非 codex 渠道 / 响应被裁剪）才算未知 —— 两者混淆会让「没开收敛」看起来像「没抓到」；
...
import { extractCodexFingerprintMode } from '../modules/codex2api/remote-sync.js';
...
test('extractCodexFingerprintMode：四档照读，字段缺失才是未知', () => {
  // 标准位置：账号平铺字段 codex_fingerprint_mode（codex 渠道账号恒有值且已归一）
  for (const mode of ['off', 'device', 'session', 'full']) {
    assert.equal(extractCodexFingerprintMode({ id: 1, codex_fingerprint_mode: mode }), mode);
  }
  // off 是**有效值**（远端确实关闭收敛），非法值归一为 off（绝不放行未知值）
  assert.equal(extractCodexFingerprintMode({ id: 1, codex_fingerprint_mode: 'bogus' }), 'off');
  assert.equal(extractCodexFingerprintMode({ id: 1, codex_fingerprint_mode: '  full  ' }), 'full');

  // 读不到：字段缺失 / 空串 / null / 非对象入参 → 一律未知，绝不代填 off
  assert.equal(extractCodexFingerprintMode({ id: 1, proxy_url: 'http://1.2.3.4:8080' }), null);
  assert.equal(extractCodexFingerprintMode({ id: 1, codex_fingerprint_mode: null }), null);
  assert.equal(extractCodexFingerprintMode({ id: 1, codex_fingerprint_mode: '' }), null);
  assert.equal(extractCodexFingerprintMode({ id: 1, codex_fingerprint_mode: '  ' }), null);
  assert.equal(extractCodexFingerprintMode(null), null);
  assert.equal(extractCodexFingerprintMode(undefined), null);
  // 已提好的代理对象（没有档位字段）也要是未知，别把代理对象误判成 off
  assert.equal(extractCodexFingerprintMode({ id: 3, name: '23', username: 'u123' }), null);
});

// ---------------------------------------------------------------- 落库语义

test('巡检废弃：用废弃当下同步过来的远端账号对象落库（含档位），零额外远端请求', async (t) => {
  const ctx = setup();
  ...
  // 代理绑定是 proxy_url、档位是平铺字段
  const remote = {
    id: 701,
    proxy_url: 'http://u123:p@1.2.3.4:8080',
...
test('远端对象缺档位字段（读不到）→ 才回落查一次远端补齐，代理不重复查', async (t) => {
  const ctx = setup();
  t.after(() => ctx.db.close());
  ctx.db.prepare('UPDATE accounts SET codex2api_account_id=701 WHERE id=1').run();
  let remoteCalls = 0;
  const { pools } = buildHarness({
    ...ctx,
    remoteSync: {
      resolveDiscardRemote: async () => {
        remoteCalls += 1;
        // 单账号接口补齐档位，代理也顺手带上（但调用方那半边优先）
        return { proxy: { id: 9, name: '12' }, codex_fingerprint_mode: 'device' };
      },
    },
  });

  const remote = { id: 701, proxy_url: 'http://u123:p@1.2.3.4:8080' };
  ...
  // 调用方给的是废弃当下那一瞬，比补查结果更可信：本地解析到的认证账号不能被补查覆盖
  assert.equal(ctx.db.prepare('SELECT discard_proxy_user FROM accounts WHERE id=1').get().discard_proxy_user, 'u123');
});
...
test('远端档位字段是 off → 记 off（透传是确定结论，不是「没抓到」）', async (t) => {
  const ctx = setup();
  t.after(() => ctx.db.close());
  const { pools } = buildHarness({ ...ctx, remoteSync: null });

  const remote = { id: 701, proxy_url: 'http://1.2.3.4:8080', codex_fingerprint_mode: 'off' };
  await pools.moveToDiscard(1, 'banned_401', '账号被封禁', { proxy: remote }).snapshot;
...
test('远端对象不带档位字段（读不到档位）→ 列留空且不写事件，不反推成 off', async (t) => {
  const ctx = setup();
  t.after(() => ctx.db.close());
...
test('废弃后又被移回主池：档位快照随废弃字段一起清空，不污染下一次废弃', async (t) => {
  ...
  await pools.moveToDiscard(1, 'manual', '手动废弃', {
    proxy: { id: 3, proxy_url: 'http://1.2.3.4:8080', codex_fingerprint_mode: 'device' },
  }).snapshot;
