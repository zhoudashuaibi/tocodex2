 ...
 * 会一路落到 500 INTERNAL，导致前端写好的友好文案不可达（如 CODEX2API_NOT_CONFIGURED）。
 */
...
async function setup(t) {
  ...
  const routes = {
    appError: () => {
      throw errors.codex2apiNotConfigured();
    },
    appErrorConflict: () => {
    ...
    legacyNotConfigured: () => {
      throw Object.assign(new Error('请先配置 codex2api'), { status: 422, code: 'CODEX2API_NOT_CONFIGURED' });
    },
    legacyStatusOnly: () => {
    ...
    legacyServerError: () => {
      throw Object.assign(new Error('上游炸了'), { status: 502, code: 'CODEX2API_UNAVAILABLE' });
    },
    plainError: () => {
...
test('AppError 按声明的状态与 code 暴露', async (t) => {
  ...
  assert.equal(notConfigured.json().error.code, 'CODEX2API_NOT_CONFIGURED');
  assert.match(notConfigured.json().error.message, /codex2api/);
...
test('错误工厂的默认值与 code 语义稳定', () => {
  ...
  assert.equal(errors.codex2apiNotConfigured().status, 422);
  assert.equal(errors.codex2apiNotConfigured().code, 'CODEX2API_NOT_CONFIGURED');
  assert.equal(errors.codex2apiNotConfigured().message, '请先配置 codex2api 后端地址与管理员密钥');
