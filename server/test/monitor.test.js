...
import { createMonitor } from '../modules/codex2api/monitor.js';
...
function setup() {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tocodex2-monitor-'));
...
function remoteAccount({ id, email, status = 'active', rateLimitedAt = null, resetAt = null, name = null, errorMessage = '401 unauthorized', concurrency = null }) {
  return {
    ...
    ...(rateLimitedAt ? { status: 'rate_limited' } : {}),
    ...(resetAt ? { reset_5h_at: resetAt } : {}),
    ...(concurrency != null ? { base_concurrency_effective: concurrency } : {}),
...
function buildMonitor({
  ...
  const client = {
    ...
    accountRateLimit: (account) => {
      const limitedNow = ['rate_limited', 'usage_exhausted'].includes(String(account?.status || '').toLowerCase());
      return { rate_limited_at: limitedNow ? (account?.last_used_at ?? null) : null, rate_limit_reset_at: account?.reset_5h_at || account?.reset_7d_at || null, limited_now: limitedNow };
    },
    accountErrorMessage: (account) => account?.error_message || '',
    accountConcurrency: (account, fallback = 0) => {
...
test('修复连败达上限：巡检不再发修复任务，暂停保留并暂停远端调度', async () => {
  ...
  ctx.db.prepare(`UPDATE accounts SET repair_fail_count=2, codex2api_account_id=55 WHERE id=?`).run(id);
  const monitor = buildMonitor({
    autoRepair: true,
...
test('级联补号：codex2api 可用够（无缺口），主池库存低于保底仍从备用池登录补库存', async () => {
  const emails = ['a@test.local', 'b@test.local', 'c@test.local'];
  for (const email of emails) insertAccount(ctx.db, ctx.crypto, { email });
  insertAccount(ctx.db, ctx.crypto, { email: 'cand@test.local', pool: 'reserve', status: 'mail_ok' });
  const monitor = buildMonitor({
...
test('主池库存保底未设置时沿用 codex2api 保底（旧行为兼容）', async () => {
...
test('主池库存保底低于 codex2api 保底：库存补完缺口后只按库存保底补', async () => {
  insertAccount(ctx.db, ctx.crypto, { email: 'ok@test.local' });
...
test('收编号：远端 error 触发完整登录修复，远端健康零动作', async () => {
  ...
  ctx.db.prepare(`UPDATE accounts SET codex2api_account_id=901, adopted_remote=1 WHERE id=?`).run(adoptedOk);
  ctx.db.prepare(`UPDATE accounts SET codex2api_account_id=902, adopted_remote=1 WHERE id=?`).run(adoptedErr);
  const monitor = buildMonitor({
    autoRepair: true,
    remoteAccounts: [
...
test('远端同邮箱重复条目只跟踪一条（优先本地关联 ID），error 计数不虚高', async () => {
  ...
  ctx.db.prepare(`UPDATE accounts SET codex2api_account_id=5 WHERE id=?`).run(id);
  const monitor = buildMonitor({
    remoteAccounts: [
