...
test('引擎集成：2FA 取件码账号 → mfa_otp 自动取码作答 + launcher 注入取件 URL', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tocodex2-pickup-'));
  ...
  process.env.TOCODEX2_PROTOCOL_SCRIPT = path.resolve(serverRoot, 'test/mock-protocol-login.mjs');
  process.env.TOCODEX2_MOCK_SCRIPT = scriptPath;
  process.env.TOCODEX2_MOCK_RESULT_PATH = '1';
  process.env.TOCODEX2_MOCK_ENV_DUMP = envDumpPath;
  ...
  try {
    ...
    delete process.env.TOCODEX2_PROTOCOL_SCRIPT;
    delete process.env.TOCODEX2_MOCK_SCRIPT;
    delete process.env.TOCODEX2_MOCK_RESULT_PATH;
    delete process.env.TOCODEX2_MOCK_ENV_DUMP;
    mockServer.close();
    db.close();
