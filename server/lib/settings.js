...
const SECRET_KEYS = new Set(['console.password', 'codex2api.config', 'sms.providers']);
...
export const DEFAULT_SETTINGS = {
  ...
  'codex2api.config': {
    ...
    group_ids: [],
    upload_defaults: {
      // 每号并发上限（→ codex2api base_concurrency_override），null=不覆盖
      base_concurrency: null,
      // 调度偏置（→ score_bias_override，-200~200，正向=优先调度），null=按余额分档
      score_bias: null,
      // 创建后跳过 codex2api 的即时上游刷新（默认开：批量上传防打卡网关）
      skip_refresh: true,
      model_whitelist: [],
      disable_auto_pause_5h: false,
      disable_auto_pause_7d: false,
      // Codex 指纹收敛（codex_fingerprint_mode）：off=透传（默认）/ device / session / full
    ...
    monitor: {
      ...
      // 补号触发口径：count=按账号数量（reserve_threshold）；resource=按 codex2api 在架号总并发 + 初始总余额
  ...
  // Team 号池：兑换服务地址 + 独立的上传默认配置（不与 codex2api.config 的共用）
  'team.config': {
