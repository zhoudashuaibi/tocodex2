...
import { normalizeCodexFingerprintMode } from '../codex2api/upload.js';
...
export function createTeamModule({ logger }) {
  return async function teamModule(app) {
    ...
    const uploader = createTeamUploader({
      db,
      crypto: app.crypto,
      client: app.codex2apiClient,
      getCodex2apiConfig: () => app.settings.get('codex2api.config'),
      ...
      const items = db
        .prepare(
          `SELECT c.*,
             ...
             (SELECT COUNT(*) FROM team_accounts a WHERE a.card_id = c.id AND a.codex2api_account_id IS NOT NULL) AS uploaded_count
    ...
    app.get('/api/v1/team/accounts', async (request) => {
      ...
      if (['1', 'true'].includes(uploaded)) {
        where.push('a.codex2api_account_id IS NOT NULL');
      } else if (['0', 'false'].includes(uploaded)) {
        where.push('a.codex2api_account_id IS NULL');
      }
      ...
      const items = db
        .prepare(
          `SELECT a.id, a.card_id, c.card_code, a.email, a.short_name, a.name, a.health_status,
             a.codex2api_account_id, a.codex2api_uploaded_at, a.created_at, a.updated_at
    ...
    app.post(
      ...
      async (request) => {
        ...
        if (!codex2apiConfig?.base_url || !codex2apiConfig?.admin_key) {
          throw errors.validation('请先在「Codex2API」页配置后端地址与管理员密钥');
        }
