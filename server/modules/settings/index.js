...
export function createSettingsModule({ logger }) {
  return async function settingsModule(app) {
    ...
    function view() {
      ...
      const codex2api = app.settings.get('codex2api.config') || {};
      return {
        ...
        join_auto_upload: Boolean(codex2api.join_auto_upload),
        sms: {
    ...
    app.put('/api/v1/settings', async (request) => {
      ...
      if (body.join_auto_upload !== undefined) {
        const current = app.settings.get('codex2api.config');
        app.settings.set('codex2api.config', { ...current, join_auto_upload: Boolean(body.join_auto_upload) });
      }
