 ...
 *
 * 取值与「主号池预估剩余余额」完全同源：codex2api 管理端账号用量
 * （client.accountUsedAmount → used_amount / consumed_amount / total_cost / usage.* /
 *   usage_stats.summary.total_cost）。
 *
 * 为什么需要快照列而不是每次现查：
 *  1. codex2api 只保留账号**当前累计**用量，不提供历史时点查询，所以「废弃那一刻的用量」
 *     只能在废弃当下抓一次；事后刷新得到的是当前累计值，UI 会标注同步时间。
 *  2. 远端账号可能被删除（远端同步只扫主号池，废弃号的 codex2api_account_id 会变陈旧），
 *     快照能保住已有数字。
 *  3. 出口代理与 Codex 指纹收敛档位同理，而且更脆：代理会被「一键更换 IP」改绑并删掉旧代理，
 *     收敛档位可以在 codex2api 账号编辑页随时改 —— 错过废弃那一刻，两者都再也查不到「当时是多少」。
 *
 * 远端解析沿用与主池预估相同的双路回退：codex2api_account_id 命中 → 否则按 email 命中。
 */

import { proxyAuthAccount } from '../../lib/sanitize.js';
import { extractRemoteProxy, extractCodexFingerprintMode } from '../codex2api/remote-sync.js';
...
/**
 * 按 id / email 建远端账号索引（语义与 modules/codex2api/remote-sync.js buildAccountIndex 一致）。
 */
...
export function resolveRemoteAccount(row, index) {
  const linked = Number(row.codex2api_account_id);
...
export function classifyDiscardUsage({ row, remote, used, lookupError = null }) {
  if (!remote) {
    return {
      ...
      reason: row.codex2api_account_id == null ? 'not_linked' : lookupError ? 'fetch_failed' : 'remote_account_not_found',
      detail: lookupError,
    };
  }
 ...
 *
 * 关键设计：**不预先拉取完整远端账号列表**。
 * 早期实现调用 `listAllOpenAiAccounts()`（分页遍历 codex2api 全部 openai 账号）来建 email 索引，
 * 代价与远端账号总数成正比：号多时单页请求 × 数十页串行，最终撞上 120s 超时，
 * 整个同步请求以 504 失败——表现为「所有账号都失败」。
 *
 * 改为按需解析（见 resolveTargetRemote）：
 *   - 有 codex2api_account_id → 直接查单账号接口（1 次请求）
 *   - 没有 ID → 才走邮箱查找，且带条数上限
 ...
 *   client 与 getClient 二选一：getClient 用于「codex2api 模块晚于本模块注册」的场景。
...
export function createDiscardUsage({
  ...
  function resolveClient() {
    ...
    if (!resolved) throw new Error('codex2api 客户端未初始化');
    return resolved;
  }
   ...
   * （否则用户无法区分「远端确实没这个号」和「codex2api 连不上」）。
   */
   ...
   * 取一个远端账号的累计已用额度。
   *
   * **必须显式查 /usage 接口**：账号对象（无论来自列表还是单账号接口）通常不含累计费用字段，
   * 累计消费在 `/api/admin/accounts/{id}/usage?days=0` 的 total_account_billed 里
   * （days=0 = 全部历史，与「废弃那一刻的累计用量」口径对齐）。
   * 再交给 client.accountUsedAmount() 按候选表取值（total_account_billed 优先，
   * 列表自带的 usage_5h/7d_detail.account_billed 次选）。
  ...
  async function fetchUsedAmount(api, remote) {
    ...
    try {
      const payload = await api.getAccountStats(remote.id, 0);
      const stats = payload?.data && typeof payload.data === 'object' ? payload.data : payload;
      // 平铺合并后走同一张候选表（total_account_billed 在顶层）
      return { used: pick({ ...remote, ...stats }) };
    } catch (error) {
  ...
  function selectTargets({ ids = null, force = false, filters = null } = {}) {
    if (Array.isArray(ids) && ids.length > 0) {
      ...
      return db
        .prepare(`SELECT id, email, codex2api_account_id, discard_used_amount_at FROM accounts WHERE pool='discard' AND id IN (${placeholders}) ORDER BY id`)
        .all(...ids);
    }
    ...
    const rows = scoped
      ? db
          .prepare(
            `SELECT id, email, codex2api_account_id, discard_used_amount_at FROM accounts ${scoped.where} ORDER BY id`,
          )
          .all(...scoped.params)
      : db
          .prepare(`SELECT id, email, codex2api_account_id, discard_used_amount_at FROM accounts WHERE pool='discard' ORDER BY id`)
          .all();
   ...
   * 废弃号池「废弃时的出口代理」快照。
   *
   * 主来源是调用方在废弃当下同步过来的远端绑定代理（codex2api 账号的 proxy_id → 代理名 + 认证账号）。
   * 兜底来源是本机 tocodex2 代理：账号最后一次任务的 jobs.proxy_id（登录/修复任务真正走的出口）。
   *
  ...
  function resolveJobProxy(accountId) {
    ...
    return {
      ...
      // 本机代理没起名字时退回 codex2api 绑定代理的展示名（余额任务走远端选路时记的）
    ...
    for (let start = 0; start < targets.length; start += concurrency) {
      ...
      for (const entry of settled) {
        ...
        items.push({
          ...
          // 失败原因带给前端：用户需要看到「是账号不在远端」还是「codex2api 连不上」
   ...
   * 会把 codex2api 打爆；串行与早先「逐条 await」的语义一致，只是不再阻塞调用方。
   */
  ...
  async function snapshotDiscardFacts(accountId, remoteAccount) {
    try {
      // 传进来的可能是「带 proxy_url 的远端账号」，也可能是已经提好的代理对象。
      // 号没绑代理时 extractRemoteProxy 返回 null → 继续走下面的回退，
      // 不能当成「解析成功但值为空」而提前收工。
      ...
      if ((!proxy || mode == null) && remoteSync?.resolveDiscardRemote) {
        ...
        if (row) {
          const resolved = await remoteSync.resolveDiscardRemote({
            accountId: row.codex2api_account_id,
            email: row.email,
          });
      ...
      // 直传路径解析出的代理没有 label（codex2api 账号只带 proxy_url，名字在代理表）：
      // 用代理索引补一次，失败不阻断（username/host 已足够归因，label 只是更好读）
      if (proxy && !proxy.name && remoteAccount?.proxy_url && remoteSync?.resolveProxyLabel) {
        try {
          proxy.name = (await remoteSync.resolveProxyLabel(remoteAccount.proxy_url)) ?? proxy.name;
        } catch {
          // label 是锦上添花，取不到就算了
        }
      }
