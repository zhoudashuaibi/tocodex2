...
import { createAutoInput, readCodex2apiExport } from './auto-input.js';
...
export function createJobsEngine({ config, db, logger }) {
  ...
  let stopped = false;
  let balanceActive = 0;
  let balanceProxyResolver = null; // codex2api 模块注入：号已上传时解析远端绑定代理
  ...
  async function runBalanceJob(job) {
    ...
    // 已上传 codex2api 的号优先用远端绑定代理查询（与线上出口 IP 一致，避免换 IP 查询触发风控）；
    ...
    if (balanceProxyResolver) {
      ...
      try {
        ...
        logger.warn({ jobId: job.id, err: error.message }, 'resolve codex2api-bound proxy failed');
      }
      if (remoteRoute?.url) {
        const label = `codex2api 代理 ${remoteRoute.proxy_name || remoteRoute.url.replace(/^[a-z0-9]+:\/\//i, '')}`;
        patchJob(job.id, { proxy_label: label });
        appendJobLog(job, `balance via codex2api-bound proxy: ${label}, remote_account=${remoteRoute.remote_id}`);
        logger.info(
          { jobId: job.id, remoteAccountId: remoteRoute.remote_id, proxy: remoteRoute.proxy_name },
          'balance job via codex2api-bound proxy',
        );
        let result = null;
        for (let attempt = 1; attempt <= BALANCE_PROXY_ATTEMPTS && !result; attempt += 1) {
          try {
            ...
            logger.warn({ jobId: job.id, attempt, err: error.message }, 'codex2api-bound proxy balance attempt failed');
            await new Promise((resolve) => setTimeout(resolve, 1500));
          }
        }
        persistBalanceResult(job, account, tokens, result, 'codex2api_proxy');
        return;
      }
    }

    // 未上传 codex2api：选路与登录一致：有可用代理先走代理（账号绑定代理 > 全局 alive 代理）。
    ...
    for (let attempt = 1; ; attempt += 1) {
      ...
      // 重试等场景可能残留 codex2api 标签：本机选路一律覆盖为空
  ...
  function handleSaveTokens(job, runtime, event) {
    try {
      const exportData = readCodex2apiExport(config.dataDir, event.path || path.join('data', 'results', `${job.id}.json`));
      ...
      // 钩子可能被包装成 async（codex2api 模块），同步异常与 Promise 拒绝都要落日志，不能静默
