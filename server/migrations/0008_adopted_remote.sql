-- 收编标记：adopt_remote 导入的主号池账号（无本地 tokens，直接关联远端 codex2api）。
...
-- 没有 codex2api_account_id，登录成功后有 tokens，均不会命中）
UPDATE accounts SET adopted_remote = 1
 WHERE pool = 'main' AND tokens_enc IS NULL AND codex2api_account_id IS NOT NULL;
